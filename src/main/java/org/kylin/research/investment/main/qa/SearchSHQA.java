...
import org.kylin.research.investment.dao.ShEHuDongDao;
...
public class SearchSHQA {
    ...
    public static void main(String[] args) {
        List<Stock> stocks = stockList.stream().filter(s -> s.getCode().startsWith("SH")).collect(Collectors.toList());
        stocks.stream().forEach(s->{
            System.out.println("----------------------"+s.getName()+"----------------------------");
            List<HuDongYiQA> stockAllQA = shEHuDongDao.getStockAllQA(s);
            shEHuDongDao.insertQA(stockAllQA);
        });
    }
